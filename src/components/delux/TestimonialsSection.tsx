...
export default function TestimonialsSection() {
  const directions = useMemo(() => getDisruptiveDirections(testimonials.length), []);
    ...
    <section id="testimonios" className="py-20 sm:py-28 bg-[#0B0B0B]" aria-label="Testimonios de clientes">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((testimonial, i) => (
            <DopamineReveal
              ...
              jitter={0.05}
            >
              <article className="bg-[#1A1A1A] border border-[#2A2A2A] p-6 sm:p-8 relative group hover:border-[#C9A227]/30 transition-all duration-500 hover:shadow-[0_0_25px_rgba(201,162,39,0.08)]">
                {/* Quote icon */}
        ...
        <div className="text-center mt-12">
          <DopamineReveal direction="scale" delay={0.3}>
            <div className="inline-flex items-center gap-3 px-6 py-3 border border-[#2A2A2A] hover:border-[#C9A227]/30 transition-colors">
              ...
              <TurboCounter end={4.9} decimals={1} duration={1800} className="text-white font-semibold" />
